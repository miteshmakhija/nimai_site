...
```
frontend/     # React (CRA) application — the deployable site
backend/      # reserved for backend services (none yet)
deployment/   # deployment configs & docs (this folder, amplify.yml)
```

All build commands below run from `frontend/` (or via Amplify's `appRoot: frontend`).
The Amplify build spec lives at [`deployment/amplify.yml`](amplify.yml) — in the Amplify
Console, connect this monorepo with **app root = `frontend`**, or paste the contents of
`amplify.yml` into the build settings.
...
> Run build commands from the `frontend/` directory: `cd frontend && npm run build`.
